...
public class DayOne 
{
	static void dayOne()
	{
		try 
		{
			...
			{
				input.add(Integer.parseInt(myReader.nextLine())); //Adds elements to arraylist "input" by reading them from the input file.
			}
			System.out.println(taskOne(input));
			System.out.println(taskTwo(input));
			myReader.close();
			input.clear();
		} 
 ...
 */
	...
	{
		int x = 0;
		for(int i = 0; i < input.size()-1; i++) //compares each element from "input" with the next one
		{
 ...
 */
	...
	{
		int x = 0;
		for(int i = 0; i < input.size()-3; i++) //compares each 3 elements from "input" with the window of next three
		{
