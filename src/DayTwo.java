 ...
 */
	...
	{
		int x = 0; //Horizontal coordinates
		int y = 0; //Vertical coordinates
		for(int i = 0; i < input.size()-1; i=i+2)
		{
			String direction = input.get(i); //Updates values based on directions
			switch (direction)
			{
			case "forward":
 ...
 */
	...
	{
		int x = 0; //Current horizontal coordinates
		int y = 0; //Current vertical coordinates
		...
		for(int i = 0; i < input.size()-1; i=i+2) //i = i+2 because every 1st element is direction and every 2nd is value
		{
			...
			switch (direction) //Updates one of values, based on dirrections
			{
			case "forward":
				x = x + Integer.parseInt(input.get(i+1));
